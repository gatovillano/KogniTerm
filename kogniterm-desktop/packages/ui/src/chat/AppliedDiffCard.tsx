...
import { AppliedDiff } from '@kogniterm/types';

export interface AppliedDiffCardProps {
  diff: AppliedDiff;
      ...
      >
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="flex min-w-0 items-center gap-2">
            <span
              className="truncate font-mono text-[12px] font-medium text-zinc-200"
              title={diff.filePath}
            >
            ...
            {diff.diffContent ? (
              diff.diffContent.split('\n').map((line, i) => {
                ...
                if (trimmed.startsWith('@@')) {
                  return (
                    <div key={i} className="text-cyan-400/80 bg-cyan-950/20 px-2 py-0.5 rounded my-0.5">
                      {line}
                    </div>
                  );
                }
                ...
                if (
                  ...
                  return (
                    <div key={i} className="text-zinc-500 font-semibold px-2">
                      {line}
                    </div>
                  );
                }
                ...
                if (line.startsWith('+')) {
                  return (
                    <div key={i} className="text-emerald-300 bg-emerald-950/30 px-2 py-0.2 rounded-sm">
                      {line}
                    </div>
                  );
                }
                ...
                if (line.startsWith('-')) {
                  return (
                    <div key={i} className="text-rose-300 bg-rose-950/30 px-2 py-0.2 rounded-sm">
                      {line}
                    </div>
                  );
                }
                ...
                return (
                  <div key={i} className="text-zinc-400 px-2">
                    {line}
                  </div>
                );
              })
            ) : (
              <span className="text-zinc-500 italic">Diff vacío</span>
            )}
          </pre>
        </div>
      )}
