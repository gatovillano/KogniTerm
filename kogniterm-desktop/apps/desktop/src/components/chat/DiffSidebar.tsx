...
export const DiffSidebar: React.FC<DiffSidebarProps> = ({ diffs }) => {
    ...
    return (
        <div className="flex h-full flex-col">
            ...
            <div className="border-b border-zinc-200 p-3 space-y-2.5 bg-zinc-50/50">
                <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-zinc-800">
                        <FileDiff size={15} className="text-indigo-600" />
                        <span>Diffs Aplicados ({diffs.length})</span>
                    </div>

                    {diffs.length > 0 && (
                        <div className="flex items-center gap-1.5 font-mono text-[10px] font-bold">
                            <span className="rounded bg-emerald-500/10 border border-emerald-500/30 px-1.5 py-0.5 text-emerald-700">
                                +{totalAdditions}
                            </span>
                            <span className="rounded bg-rose-500/10 border border-rose-500/30 px-1.5 py-0.5 text-rose-700">
                                -{totalDeletions}
                            </span>
                        </div>
                    )}
                ...
                {diffs.length > 0 && (
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full rounded-lg border border-zinc-200 bg-white py-1.5 pl-8 pr-3 font-mono text-xs text-zinc-800 placeholder-zinc-400 outline-none transition-colors focus:border-indigo-500"
                        />
                    </div>
                )}
            </div>
            ...
            <div className="goose-scrollbar flex-1 overflow-y-auto p-3 space-y-2.5">
                {diffs.length === 0 ? (
                    <div className="flex h-full flex-col items-center justify-center gap-2 text-center p-6 text-zinc-400">
                        <FileDiff size={28} className="text-zinc-400 opacity-60" />
                        <p className="text-[13px] font-medium text-zinc-700">Sin diffs aplicados</p>
                    ...
                    <div className="flex flex-col items-center justify-center p-6 text-center text-zinc-500">
                        <Filter size={20} className="mb-1 text-zinc-400" />
