...
from kogniterm.core.tools.tool_manager import ToolManager
...
def test_tool_manager_with_skills():
    """Test ToolManager with SkillManager integration"""
    print("\n=== Testing ToolManager with SkillManager ===")
    
    try:
        llm_service = LLMService(use_multi_provider=False)
        print(f"✅ LLMService initialized successfully")
        
        print(f"Total tools available: {len(llm_service.tool_manager.get_tools())}")
        
        print("\nTools registered in tool_map:")
        for tool_name, tool in llm_service.tool_manager.tool_map.items():
            print(f"  - {tool_name}")
