#!/usr/bin/env python3
"""
Script to patch the LLMService with enhanced tool call parsing
"""
...
def patch_llm_service():
    ...
    with open('kogniterm/core/llm_service.py', 'r') as f:
        content = f.read()
    
    # Enhanced method to replace the old one
    enhanced_method = '''    def _parse_tool_calls_from_text(self, text: str) -> List[Dict[str, Any]]:
        """
...
'''
    ...
    start_pattern = r'\s+def _parse_tool_calls_from_text\(self, text: str\) -> List\[Dict\[str, Any\]\]:'
    end_pattern = r'\s+return tool_calls\s*'
    ...
    # Find the return statement that ends the method
    from_pos = start_match.end()
    return_match = re.search(end_pattern, content[from_pos:])
    if not return_match:
        print("Could not find end of method")
        return False
    
    end_pos = from_pos + return_match.end()
    
    # Replace the method
    new_content = content[:start_match.start()] + enhanced_method + '\n\n' + content[end_pos:]
    
    # Write the modified content
    with open('kogniterm/core/llm_service.py', 'w') as f:
        f.write(new_content)
    
    print("Successfully patched LLMService with enhanced tool call parsing")
    return True
