## 07-09-25 Corrección de NameError para deque en LLMService
 ...
 Descripción general: Se corrigió el error `tool_call() missing 1 required keyword-only argument: 'id'` que ocurría al cargar el historial de conversación, específicamente al reconstruir los objetos `ToolMessage`. Este error se debía a que el atributo `id` de `ToolMessage` no se estaba persistiendo ni cargando correctamente.
 
 - **Punto 1**: Se modificó la función `_load_history` en `kogniterm/core/llm_service.py` para incluir el atributo `id` de los `ToolMessage` al serializar el historial en `kogniterm_history.json`.
 ...
 ---
 ## 08-09-25 Implementación de Selección Dinámica de Proveedores LLM
 
 Descripción general: Se refactorizó el sistema para permitir la selección dinámica de proveedores de LLM (Gemini, OpenAI, OpenAI-compatible) y su configuración flexible mediante variables de entorno. Se definieron interfaces y clases para cada proveedor, y se actualizó `LLMService` para actuar como una fachada.
 
 - **Punto 1 (kogniterm/core/llm_providers.py)**:
     - Se definió `AbstractLLMProvider` como una clase abstracta con el método `generate_content`.
     - Se crearon `GeminiService` y `OpenAIService` que heredan de `AbstractLLMProvider` y encapsulan la lógica de interacción con sus respectivas APIs.
     - Se implementó `OpenAICompatibleProvider` para proveedores compatibles con la API de OpenAI, utilizando `LLM_API_ENDPOINT` como `base_url`.
     - Se creó la función `get_llm_provider` para instanciar el proveedor adecuado según la variable de entorno `LLM_PROVIDER`.
 - **Punto 2 (kogniterm/core/llm_service.py)**:
     - Se modificó la clase `LLMService` para que en su inicialización lea `LLM_PROVIDER` y utilice `get_llm_provider` para instanciar el proveedor de LLM.
     - Se eliminó la lógica de conversión de herramientas específica de Gemini y la inicialización directa del modelo Gemini, delegando estas responsabilidades al proveedor seleccionado.
     - Se ajustó el método `invoke` para delegar la llamada a `generate_content` del proveedor instanciado, pasando el historial y las herramientas.
 - **Punto 3 (kogniterm/core/agents/bash_agent.py)**:
     - Se añadió un método `copy()` a `AgentState` para permitir la creación de copias del estado, utilizando `dataclasses.replace()`.
     - Se ajustó `call_model_node` para manejar la respuesta del LLM (que ahora es un `AIMessage`) y añadirla al historial.
     - Se modificó `explain_command_node` para extraer el comando y la explicación del `AIMessage` de forma segura.
     - Se ajustó `should_continue` para una lógica de transición más robusta basada en `AIMessage` y `tool_calls`.
