...
### ⚙️ Refuerzo del Protocolo de Seguimiento de Tareas (task_tracker) y Activación de Pensamiento en Antigravity
- **Cambios**:
  ...
  - **Activación de Pensamiento Nativo en Antigravity**: Se modificó `completion` en [antigravity_client.py](file:///home/gato/Proyectos/Gemini-Interpreter/kogniterm/core/antigravity_client.py) para inyectar la configuración `thinkingConfig` (con un `thinkingBudget` configurable mediante la variable de entorno `KOGNITERM_THINKING_BUDGET` o por defecto a 2048) en el objeto `generationConfig` enviado a la API interna de Google para aquellos modelos que soporten razonamiento (e.g. `gemini-2.5-pro`, `gemini-3-pro`, o aquellos que tengan "thinking" en el nombre), resolviendo así la falta de flujo de pensamientos del LLM bajo este proveedor.
  - **Pruebas Unitarias**: Se corrigió el desempaquetado de `map_messages` en el test existente de [test_antigravity_integration.py](file:///home/gato/Proyectos/Gemini-Interpreter/tests/test_antigravity_integration.py) y se añadió una nueva prueba unitaria `test_thinking_config_injection` para validar la correcta inyección de la configuración de pensamiento nativo en el payload de petición.
