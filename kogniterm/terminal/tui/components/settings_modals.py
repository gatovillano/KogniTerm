...
class TextualConfirmModal(BaseModal):
    """Command palette para confirmación binaria (Sí/No)."""

    DEFAULT_CSS = BaseModal.CSS + """
    #palette-confirm-body {
        padding: 1 4 1 4;
    }
    #palette-confirm-text {
        margin-bottom: 1;
    }
    #palette-confirm-footer {
        height: 3;
        padding: 0 4;
        content-align: right middle;
    }
    ...
    def __init__(self, title, text, confirm_label="Sí", cancel_label="No"):
        super().__init__()
        self.dialog_title = title
        self.dialog_text = text
    ...
    def _build_css(self) -> str:
        ...
        """ % (p.TEXT_SECONDARY, p.GRAY_700, p.TEXT_PRIMARY,
               p.GRAY_600, p.TEXT_PRIMARY, p.PRIMARY, p.GRAY_900)

    def compose(self) -> ComposeResult:
        with Vertical(id="palette-shell"):
            with Horizontal(id="palette-header"):
                yield Label(self.dialog_title, id="palette-title-label")
            with Vertical(id="palette-confirm-body"):
                yield Label(self.dialog_text, id="palette-confirm-text", markup=True)
            with Horizontal(id="palette-confirm-footer"):
                yield Label(self.cancel_label, classes="confirm-btn", id="btn-cancel")
                yield Label(self.confirm_label, classes="confirm-btn", id="btn-confirm")
    ...
    def on_click(self, event):
        ...
        if isinstance(widget, Label):
            if widget.id == "btn-confirm":
                self.dismiss(True)
            elif widget.id == "btn-cancel":
                self.dismiss(False)
