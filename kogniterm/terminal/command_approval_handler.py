...
class CommandApprovalHandler:
    ...
    def _ensure_unified_diff(self, file_path: str, diff_or_content: str, original_content: str = None) -> str:
        """
        ...
        Si ``original_content`` se proporciona (antes de la edición), se usa como
        base para generar el diff en lugar de leerlo del disco. Esto es importante
        porque después de aplicar la edición el archivo ya contiene el contenido
        modificado y leerlo daría un diff vacío.
        """
            ...
            if is_plan_confirmation:
                ...
                if tool_name in ["file_update_tool", "file_update"]:
                    ...
                    if should_render_applied_diff:
                        ...
                        # Generar un diff unificado real a partir del contenido original
                        # y del nuevo contenido que la herramienta aplicó al archivo.
                        if not diff_content or diff_content == original_tool_args.get("content", ""):
                            new_content = original_tool_args.get("content", "") or normalized_result.get("message", "")
                            diff_content = self._ensure_unified_diff(file_path, new_content, original_file_content)
                    ...
                    if not diff_content and file_path and original_tool_args.get("content"):
                        diff_content = self._ensure_unified_diff(file_path, original_tool_args.get("content", ""), original_file_content)
