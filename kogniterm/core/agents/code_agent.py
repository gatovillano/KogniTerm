...
def _render_applied_diff_if_needed(terminal_ui, tool_name: str, tool_args: dict, output_str: str) -> None:
    """
    Muestra el diff aplicado en la TUI después de que una herramienta de
    edición de archivos haya sido ejecutada con éxito.
    """
    if not terminal_ui or tool_name not in _FILE_EDIT_TOOLS:
        return
...
def execute_single_tool(tc, llm_service, terminal_ui, interrupt_queue):
    ...
    try:
        ...
        _render_applied_diff_if_needed(terminal_ui, tool_name, tool_args, output_str)
