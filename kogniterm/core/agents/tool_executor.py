...
class ToolExecutor:
    ...
    def _handle_special_tools(tool_name, output, llm_service):
        if tool_name in ['refresh_tools', 'skill_factory'] and hasattr(llm_service, 'skill_manager'):
            ...
            if hasattr(llm_service, 'sync_tools'):
                ...
                logger.info(f"[{tool_name}] tool_map sincronizado tras refresh. Herramientas activas: {list(llm_service.tool_map.keys())}")

    @staticmethod
    def _render_cli_result(tool_name, output):
        display = output[:1000] + ("..." if len(output) > 1000 else "")
        console.print(Panel(Markdown(display) if "```" in display else Text(display), 
                          title=f"[bold green]✅ {tool_name}[/bold green]", border_style="green"))

    @staticmethod
    def execute_tool_node(state: AgentState, llm_service: LLMService, terminal_ui: Optional[Any] = None, interrupt_queue: Optional[queue.Queue] = None):
...
def should_continue(state: AgentState) -> str:
    """Lógica común de transición para agentes."""
    from langgraph.graph import END
    if state.critical_loop_detected or state.command_to_confirm or state.file_update_diff_pending_confirmation:
        return END
    last_message = state.messages[-1]
